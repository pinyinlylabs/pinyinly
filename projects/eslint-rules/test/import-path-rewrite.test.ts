...
ruleTester.run(`import-path-rewrite`, importPathRewrite, {
  valid: [
    // Paths that don't match any pattern remain unchanged
    {
      code: `import foo from 'foo';`,
      options: [{ patterns: [{ from: String.raw`^#(.+)\.ts$`, to: `@/$1` }] }],
    },
    {
  ...
  invalid: [
    // Basic rewrite: #util/foo.ts -> @/util/foo
    {
      ...
      errors: [
        {
          message: `Import path "#util/foo.ts" should be rewritten to "@/util/foo"`,
        },
      ],
    ...
    // Also with named imports
    {
      ...
      errors: [
        {
          message: `Import path "#components/bar.ts" should be rewritten to "@/components/bar"`,
        },
      ],
    ...
    // With export declarations
    {
      ...
      errors: [
        {
          message: `Import path "#models/index.ts" should be rewritten to "@/models/index"`,
        },
      ],
    ...
    // With named exports
    {
      ...
      errors: [
        {
          message: `Import path "#constants/values.ts" should be rewritten to "@/constants/values"`,
        },
      ],
    ...
    // Multiple patterns - first match wins
    {
      ...
      options: [
        {
          patterns: [
            { from: String.raw`^#api/(.+)\.ts$`, to: `@/services/$1` },
            { from: String.raw`^#(.+)\.ts$`, to: `@/$1` },
          ],
        },
      ],
      errors: [
        {
          message: `Import path "#api/client.ts" should be rewritten to "@/services/client"`,
        },
      ],
    ...
    // Quote styles are preserved
    {
      ...
      errors: [
        {
          message: `Import path "#styles/main.ts" should be rewritten to "@/styles/main"`,
        },
      ],
