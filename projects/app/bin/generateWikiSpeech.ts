...
async function convertMp3ToM4a(mp3Path: string, m4aPath: string): Promise<void> {
...
async function saveAudioFile(
  ...
  const finalBaseFileName = baseFileName || phrase.replace(/[^\u4e00-\u9fff\w]/g, ""); // Keep only Chinese characters and word characters
  
  // Save as mp3 first
  const mp3FileName = `${finalBaseFileName}-${voice}.mp3`;
  const mp3FilePath = `${outputDir}/${mp3FileName}`;
  
  await mkdir(dirname(mp3FilePath), { recursive: true });
  ...
  if (format === "m4a") {
    ...
    try {
      await convertMp3ToM4a(mp3FilePath, m4aFilePath);
      ...
      console.warn(`Failed to convert to m4a, keeping mp3 format: ${error}`);
...
async function generateWikiSpeech(options: GenerateAudioOptions): Promise<void> {
  ...
  for (const voice of voices) {
    try {
      ...
      const filePath = await saveAudioFile(audioBuffer, phrase, voice, outputDir, format, baseFileName);
