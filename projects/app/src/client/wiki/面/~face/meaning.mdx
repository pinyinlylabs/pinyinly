export const Component = function Component(props) {
  return <>Component: {props.children}</>;
};

export const Component2 = function Component(props) {
  return <>Component2: {props.children}</>;
};

The most basic meaning of **面** is the **face** — your actual physical face, or someone’s
figurative “face” in a social or emotional sense.

**Examples**:

- 她的**面**很漂亮。(_Her face is beautiful._)
- 他低下了**面**，不敢看我。(_He lowered his face, not daring to look at me._)
- 保住**面**子是他最大的在意。(_Saving face is what he cares most about._)

This is often used with the word **面子** (miànzi), meaning **reputation**, **dignity**, or **saving
face**, especially in Chinese social culture.

This is the most literal meaning: **face**. It's used for both the physical face and for ideas like
_saving face_ or _losing face_, which are culturally important in Chinese.

<Example hanzi="她的脸上带着微笑。" translated="There was a smile on her face." />
<Example
  hanzi="我不想在大家面前丢面子。"
  translated="I don't want to lose face in front of everyone."
/>
<Example
  hanzi="他长得一副苦瓜脸，一点面子都没有。"
  translated="He always looks so bitter — no dignity at all."
/>
<Example hanzi="请当面告诉我。" translated="Please tell me face-to-face." />

prefixed <Example2><Hanzi>他长得一副苦瓜脸，一点**面**子都没有。</Hanzi> <Translated>He always looks
so bitter — no dignity at all.</Translated></Example2>

<Example2>
  <Hanzi>他长得一副苦瓜脸，一点**面**子都没有。</Hanzi>
  <Translated>He always looks so bitter — no dignity at all.</Translated>
</Example2>

<Highlight>面</Highlight> here shows up in words like **面子** (dignity) and **当面** (in person).
It's about **presentation**, **image**, and the **front** people see.
