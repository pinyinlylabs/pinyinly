...
const syncEntities = [
  ...
  makeSyncEntity(
    `settingHistory`,
    schema.settingHistory,
    (id, e) => e.marshalKey({ id }),
    (db, ids) =>
      db.query.userSettingHistory.findMany({
        where: (t) => inArray(t.id, ids),
      }),
    (db, userId) =>
      db
        .select({
          map: json_agg(
            json_build_object({
              id: s.userSettingHistory.id,
              xmin: pgXmin(s.userSettingHistory),
            }),
          ).as(`settingHistory`),
        })
        .from(s.userSettingHistory)
        .where(eq(s.userSettingHistory.userId, userId))
        .as(`settingHistory`),
  ),
];
