...
import { ansiDim, ansiReset, ansiYellow, getLoudnorm } from "./helpers";

const projectRootDir = path.join(import.meta.dirname, `../..`);
const wikiDir = path.join(projectRootDir, `src/client/wiki`);

describe(`speech loudness is within allowed tolerance`, async () => {
  ...
  for await (const filePath of fs.glob(audioGlob)) {
    ...
    it(projectRelPath, async () => {
      const loudnorm = await getLoudnorm(filePath);
