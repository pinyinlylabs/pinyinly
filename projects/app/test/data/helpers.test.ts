...
describe(`parseRelativeTimeShorthand suite`, () => {
...
describe(`parseDurationShorthand suite`, () => {
  test.for([
    [`30s`, { seconds: 30 }],
    [`+30s`, { seconds: 30 }],
    [`-30s`, { seconds: -30 }],
...
describe(`parseHistoryCommand suite`, () => {
  test(`sleep`, () => {
  ...
  test(`skill review with mistakes`, () => {
    ...
    expect(parseHistoryCommand(`❌ he:分:divide (xxx)`)).toMatchInlineSnapshot(`
          [
            {
              "kind": "skillReview",
              "mistake": {
...
test(`rankExampleHanziCandidates`, () => {
  const usageIndex = (
...
describe(`pickExampleHanziForPinyinUnit`, () => {
  test.for(
    [
      `chán 单`,
      `là 落`,
  ...
  )(`Expected NOT to match: %s → %s`, async ([pinyinUnit, hanziCharacter]) => {
...
test(`getHanziCharacterToPinyinUnit`, async () => {
