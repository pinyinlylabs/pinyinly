...
describe(`normalizePinyinText fixtures`, () => {
  // Rules: (from https://en.wikipedia.org/wiki/Pinyin)
  // 1. If there is an a or an e, it will take the tone mark
  // 2. If there is an ou, then the o takes the tone mark
  // 3. Otherwise, the second vowel takes the tone mark
...
describe(`splitPinyinUnit suite`, () => {
...
describe(`pinyinUnitSuggestions suite`, () => {
  test(`fixtures`, () => {
      ...
      expect({
        ...
        results:
          result == null
            ? null
            : result.units.map(
                (x) => `${result.from}-${result.to}:${x.pinyinUnit}:${x.tone}`,
              ),
      }).toEqual({
        query,
        results,
...
describe(`pinyinUnitCount suite`, () => {
  test.for([
    [拼音`hǎo`, 1],
    [拼音`nǐ hǎo`, 2],
    [拼音`nǐ  hǎo`, 2],
