...
describe(
  `parseCssColorOrThrow suite` satisfies HasNameOf<typeof parseCssColorOrThrow>,
  async () => {
    test(`fixtures`, () => {
      const fixtures = [
        [`rgb(1 2 3)`, [1, 2, 3, 1]],
...
describe(
  `parseScalar suite` satisfies HasNameOf<typeof parseScalar>,
  async () => {
    test(`valid fixtures`, () => {
...
describe(
  `parseHexColor suite` satisfies HasNameOf<typeof parseHexColor>,
  async () => {
    test(`valid fixtures`, () => {
...
describe(
  `parseRiveColorPropertyName suite` satisfies HasNameOf<
    typeof parseRiveColorPropertyName
  >,
  async () => {
    test(`no alpha suffix`, () => {
      expect(parseRiveColorPropertyName(`fg`)).toEqual({
        name: `fg`,
        alpha: 1,
      });
    });
