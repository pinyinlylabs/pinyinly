...
describe(`buildLocationPopulateSetDescriptionPrompt`, () => {
  test(`builds a single-description prompt from location spec and target set`, () => {
    const prompt = buildLocationPopulateSetDescriptionPrompt({
      locationSpec: makeLocationSpecWithDetail(`Pirate ship`),
      setKey: `below`,
    });
    ...
    expect(fmtChatPromptForSnapshot(prompt)).toMatchInlineSnapshot(`
        ...
        ---------------------
        <input>
        {"location":{"location":"Pirate ship","recognitionHooks":["mast","bow","anchor"],"designRules":["Keep the hull dominant in the composition."]},"locationSet":{"name":"Below","purpose":"below purpose","props":["below prop"],"designRules":["below design rule"],"canonicalFraming":"below canonical framing"}}
        </input>
        =====================
        ",
          "model": "gpt-5.4",
